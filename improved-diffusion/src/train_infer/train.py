...
from src.data.text_datasets import load_data_text
from src.models.diffusion.modules.resample import create_named_schedule_sampler
...
from src.utils.test_util import get_weights, compute_logp
from src.models.diffusion.modules.rounding import load_models, load_tokenizer
...
def main():
    ...
    # model.cuda() #  DEBUG **
    os.system(f"echo '{model}' > model_architecture_debug.log")
    os.system(f"echo '{diffusion}' > diffusion_architecture_debug.log")
    ...
    if args.modality == 'image':
        ...
        next(data)
        ##########################################################
        #                 WHERE ROUNFING MAY APPLY
        ##########################################################
        """
        args.modality = used dataset nickname (zuco)
        args.experiment: 'random', 'random1', 'random_up_proj', 'glove' random no-rep=gpt2gen, no-zipfs, has-rep=regular, rm-window-rep
        args.model_name_or_path
        args.in_channel: 16 = nb of inputs in the model
        args.checkpoint_path = path to where all chkpts are stored
        """
        ...
        os.system(f"echo '{model2}' > debug.log")
        ##########################################################
        #                 WHERE ROUNFING MAY APPLY
        ##########################################################
    ...
    def get_mapping_func(args, diffusion, data):
        ...
        mapping_func = partial(compute_logp, args, model3)  #model3.cuda())
